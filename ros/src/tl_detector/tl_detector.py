...
class TLDetector(object):
    def __init__(self):
        ...
        # consider using '/image_raw' instead? to avoid lose of data...
    ...
    def base_waypoints_cb(self, lane):
        ...
        if not self.wpt_tree:
            self.wpt2d = [[wpts.pose.pose.position.x, wpts.pose.pose.position.y]
                          for wpts in self.waypoints]
            self.wpt_tree = KDTree(self.wpt2d)
    ...
    def image_cb(self, msg):
        ...
        times till we start using it. Otherwise the previous stable state is
        used.
        '''
        if self.state != state:
            self.state_count = 0
    ...
    def get_closest_waypoint_idx(self, x, y):
        ...
        """
        if self.wpt_tree:
            return self.wpt_tree.query([x,y], 1)[1]
        else:
            # if KDTree has not been defined yet...
            return 0
